				...
				<form action="<c:url value="/clazz/clazz/home"/>" method="post"
					...
					<fieldset>
						<div class="form-group">
							<div class="col-md-2">
							     <select name="EQ|pkg.$id" class="form-control">
							       <option value="0"><fmt:message key="global.select"/><fmt:message key="clazz.pkg"/></option>
							       <c:forEach items="${pkgs}" var ="pkg">
							         <option value="${pkg.id}"<c:if test="${pkg.id eq entity.pkg.id}"> selected</c:if>>${pkg.name}</option>
							       </c:forEach>
							     </select>
							</div>
							<div class="col-md-2">
								<input type="text" name="EQ|longName" value="${searchParams.longName}"
